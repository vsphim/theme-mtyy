let zh_default = 'n',zh_choose = 'n';
const int = false;//用户输入转换  false繁体转简体  true简体转繁体 undefined 不转换用户输入的文字
...
function transChinese(text) {
    if(int === undefined){
        return text;
    }
    const sourceChars = int ? zh_s : zh_t;
    const targetChars = int ? zh_t : zh_s;
    let result = '';
    for (let i = 0; i < text.length; i++) {
        const char = text.charAt(i);
        const index = sourceChars.indexOf(char);
        if (index !== -1) {
            result += targetChars.charAt(index);
        } else {
            result += char;
        }
    }
...
//初始化
function zh_init() {
    let lang_bnt = $(".lang-bnt");
    lang_bnt.click(function(){
        if($(this).data("id") == "1"){
            $(this).data("id",'2').text("简");
            zh_tran("t");
        }else{
            $(this).data("id",'1').text("繁");
            zh_tran("s");
        }
    });
    let get_lang = window.localStorage.getItem('zh_choose');
    if (get_lang) {
        zh_choose = get_lang;
        if(zh_choose === 't'){
            lang_bnt.data("id",'2').text("简");
        }else{
            lang_bnt.data("id",'1').text("繁");
        }
    }
    $(function(){
        if(lang_bnt.data("id") == "1"){
            lang_bnt.data("id",'1').text("繁");
            zh_choose = "s";
        }else{
            lang_bnt.data("id",'2').text("简");
